...
void timestep(double F, double K, double diffU, double diffV) {
  for (int i = 0; i < H; i++) {
    for (int j = 0; j < W; j++) {
      ...
      double uvv = u*v*v;     
      double lapU = (.2*U[up][j] + .2*U[down][j] + .2*U[i][left] + .2*U[i][right]
                     + .05*U[up][left] + .05*U[up][right] + .05*U[down][left] + .05*U[down][right]
                     - 1*u);
      double lapV = (.2*V[up][j] + .2*V[down][j] + .2*V[i][left] + .2*V[i][right]
                     + .05*V[up][left] + .05*V[up][right] + .05*V[down][left] + .05*V[down][right]
                     - 1*v);
